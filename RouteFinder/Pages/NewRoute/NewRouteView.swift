...
struct NewRouteView: View {
    @StateObject private var newRouteViewModel = NewRouteViewModel()
    ...
    var body: some View {
        NavigationStack {
                ...
                if isSearching {
                    TextField("Search airlines...", text: $searchText)
                        ...
                        .onChange(of: searchText) { _, newValue in
                            newRouteViewModel.setQuery(query: newValue)
                        }
                        .focused($isSearchFocused)
     
                }
                
                if newRouteViewModel.isLoading {
                    List(0..<10, id: \.self) { _ in
                        SkeletonRow()
                    }
                } else {
                    List {
                        ForEach(newRouteViewModel.items) { airline in
                            NavigationLink(destination: RouteCriteriaView(airlineId: airline.id, airlineIata: airline.iataCode)) {
                                HStack(spacing: 12) {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: {
                        ...
                        if !newRouteViewModel.query.isEmpty {
                            newRouteViewModel.setQuery(query: "")
                            searchText = ""
                        }
                    }) {
            ...
            .onAppear {
                newRouteViewModel.loadItems()
            }
        }
    }
}

private class NewRouteViewModel: ObservableObject {
    var items: [Airline] = []
